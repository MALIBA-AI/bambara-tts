...
class MalianTTS:
    ...
    def __init__(
        self,
        model_id: str = Settings.models_repo,
        languages: list[str] = Settings.models,
    ) -> None:
    ...
    def _load_models(self) -> None:
        if any(lang not in Settings.models for lang in self.languages):
            raise ValueError(
                f"Unsupported language in {self.languages}. Available: {Settings.models}"
            )

        for lang in self.languages:
            self.models[lang] = VitsModel.from_pretrained(
                self.model_id, subfolder=f"{Settings.models_subfolder}/{lang}"
            ).to(self.device)
    ...
    def synthesize(
        self, text: str, language: Optional[str] = None, output_filename: str = None
    ) -> InferenceOutput:
        """
        ...
        """
        language = (
            language or self.languages[0]
        )  # Why the first one: if the user load only one model so no need to specify the language

        if language not in self.models:
            raise ValueError(
                f"Unsupported language: {language}. Available: {self.languages}"
            )
