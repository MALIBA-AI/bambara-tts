...
from maliba_ai.settings.mt.nllb import Languages, Language
...
def test_nllb_bambara_translator(src_lang : Language, tgt_lang : Language):
    ...
    for data_sample in inference_data_samples('MT'):
        ...
        assert isinstance(output, TranslationOutput) 
        assert isinstance(output.translation, str) or (isinstance(output.translation, list) and all(isinstance(item, str) for item in output.translation))
...
if __name__ == "__main__":
    ...
    test_nllb_bambara_translator(bambara, french)
